...
out_folder=samples/openapi3/server/petstore/python-aws-apigateway-dynamodb
resources=modules/openapi-generator/src/main/resources/python-aws-apigateway-dynamodb

# if you've executed sbt assembly previously it will use that instead.
export JAVA_OPTS="${JAVA_OPTS} -Xmx1024M -DloggerPath=conf/log4j.properties -Dservice"
ags="generate -t $resources -i $input -g python-aws-apigateway-dynamodb -o $out_folder $@"
